 ...
 */
package com.alibaba.csp.switchcenter.spi;
...
public final class SpiLoader {
    ...
    public static <T> T loadFirstInstance(Class<T> clazz) {
        if (clazz == null) {
            throw new IllegalArgumentException("SPI class cannot be null");
        }
        try {
            ...
            SwitchRecordLog.warn("[SpiLoader] ERROR: loadFirstInstance failed", t);
            t.printStackTrace();
            return null;
        }
    }
    ...
    /**
     * Load the first-found specific SPI instance (excluding provided default SPI
     * class). If no other SPI implementation found, then create a default SPI
     * instance.
     *
     * @param clazz        class of the SPI interface
     * @param defaultClass class of the default SPI implementation (if no other
    ...
    public static <T> T loadFirstInstanceOrDefault(Class<T> clazz, Class<? extends T> defaultClass) {
        ...
        if (defaultClass == null) {
            throw new IllegalArgumentException("default SPI class cannot be null");
        }
        ...
        try {
            ...
            SwitchRecordLog.warn("[SpiLoader] ERROR: loadFirstInstanceOrDefault failed", t);
            t.printStackTrace();
            return null;
        }
    }
    ...
    public static <T> T loadHighestPriorityInstance(Class<T> clazz) {
        try {
            ...
            for (T spi : serviceLoader) {
                ...
                SwitchRecordLog.info("[SpiLoader] Found {0} SPI: {1} with order " + order, clazz.getSimpleName(),
            ...
            SwitchRecordLog.warn("[SpiLoader] ERROR: loadHighestPriorityInstance failed", t);
            t.printStackTrace();
            return null;
        }
    }
    ...
    public static <T> List<T> loadInstanceList(Class<T> clazz) {
        try {
            ...
            SwitchRecordLog.warn("[SpiLoader] ERROR: loadInstanceListSorted failed", t);
            t.printStackTrace();
    ...
    public static <T> List<T> loadInstanceListSorted(Class<T> clazz) {
        try {
            ...
            List<SpiOrderWrapper<T>> orderWrappers = new ArrayList<SpiOrderWrapper<T>>();
            for (T spi : serviceLoader) {
                int order = SpiOrderResolver.resolveOrder(spi);
                // Since SPI is lazy initialized in ServiceLoader, we use online sort algorithm
                // here.
