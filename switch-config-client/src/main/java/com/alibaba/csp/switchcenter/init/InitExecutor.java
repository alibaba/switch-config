 ...
 */
package com.alibaba.csp.switchcenter.init;
...
import com.alibaba.csp.switchcenter.spi.ServiceLoaderUtil;
...
public final class InitExecutor {
	...
	/**
	 * If one {@link InitFunc} throws an exception, the init process will
	 * immediately be interrupted and the application will exit.
	 *
	...
	public static void doInit() {
		...
		try {
			...
			for (InitFunc initFunc : loader) {
				SwitchRecordLog.info("[InitExecutor] Found init func: " + initFunc.getClass().getCanonicalName());
				insertSorted(initList, initFunc);
			}
			for (OrderWrapper w : initList) {
				w.func.init();
				SwitchRecordLog.info(String.format("[InitExecutor] Executing %s with order %d",
						w.func.getClass().getCanonicalName(), w.order));
			}
		} catch (Exception ex) {
			SwitchRecordLog.warn("[InitExecutor] WARN: Initialization failed", ex);
			ex.printStackTrace();
		}
	}
